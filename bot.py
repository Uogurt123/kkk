...
TOKEN = '81775506881:AAH...' # Твій токен (Railway підтягне його з Variables, якщо в коді порожньо)
ADMIN_ID = 000000000 # Твій ID
...
async def press_about(callback: CallbackQuery):
    await callback.message.answer("бот нічого не вміє і не працює зроблений тільки для мене і ще одного бидла (жарт)  🤫")
...
async def press_payment_done(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        notification_text = (
            ...
            f"💎 <b>Кількість зірок:</b> {stars_amount}\n\n"
            f"Перевірте банківський додаток!"
        )
